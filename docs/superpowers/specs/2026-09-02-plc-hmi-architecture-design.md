...
流程（PLC-HMI-010 变更：发送即确认，固定延时收敛）：

1. 发送 M103 脉冲。HMI 只写 M103；不再写 M104，也不再等待 M1=1（用户决策 U4）。
2. 显示复位中（单一发送阶段），不监视 M50/M61/M14/D110。
3. 从 M103 脉冲提交起固定 200 ms 后收敛到唯一终端结果 `复位完成`（用户决策 U1/U2）。
4. 复位结果与 M50、M61（M9）、M14、D110 无关：这些位/寄存器的任意取值都不改变结果（用户决策 U3）。
5. 保留可见的失败路径：管理员权限、M3=0 互锁、受限模式、重复/进行中拒绝、M103 脉冲提交失败、通讯中断，均各自收敛为可见的终端结果，绝不乐观成功。
6. 任何已接受的复位都不得无限期处于待确认：每条路径恰好到达一个终端状态。

M103 同时清除 M44 和 M45（以及锁存故障 M14/D110）。物理回原点结果由普通状态显示（M8/M9，顶栏指示灯）呈现，不再作为 HMI 复位结果判据。

> 背景：权威 PLC 梯形图从不置位 M50（全部 11 个 SBR 文件仅 RST M50，CrossTable.crs
> 记录 M50 写入用途仅为 RST），因此旧的 `M61=1 && M50=0` 确认分支在现场不可达，只能
> 收敛到防御性超时。HMI 不再依赖 M50/M61 判定复位结果（见 .ai/reports/PLC-HMI-010-debug.yaml）。
