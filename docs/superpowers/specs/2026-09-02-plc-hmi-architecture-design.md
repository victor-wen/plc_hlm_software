...
流程（PLC-HMI-011 最小修订，保留 PLC-HMI-010 的固定延时结果语义）：

1. 发送 M103 脉冲；不写 M104，也不等待 M1=1。
2. M103 完成写 1、保持至少 100 ms、写 0，并取得关联成功结果后，以普通保持线圈写入方式写 M50=1 一次。M50 不是脉冲，不周期重写。
3. M103 或 M50 的提交/完成失败、通讯中断、网关替换或关闭都必须可见失败，不得被固定延时覆盖。
4. 从 M103 提交起至少 200 ms，且 M103 与 M50 的关联传输结果均成功后，收敛到唯一成功结果 `复位完成`。
5. M50 写入沿用 IPlcGateway 的关联写后回读，确认 M50=1 只证明命令送达，不代表回原点完成；成功结果不使用普通快照中的 M50、M61（M9）、M14、D110 作为回原点结果判据。
6. PLC 负责正常回原点过程中的 M50 清零。在线注销、会话超时和正常退出时，HMI 只尽力写一次 M50=0；该清零允许终止或抑制正在进行的回原点请求，不报告回原点完成，离线后不重放、不盲重试。
7. 任何已接受的复位都不得无限期处于待确认：每条路径恰好到达一个终端状态。

M103 清除既有复位/故障状态后，M50=1 启动回原点。PLC 工程师已确认 M50 是上位机可写启动位；梯形图中的 RST M50 是 PLC 清零路径，而不是只读证据。无 PLC 的开发环境允许先完成代码、模拟器和自动化验证，但真实 H3U 必须在任何实机连接、部署或现场发布前确认单次 M50=1 能启动 M51/M52/M53 阶段并可到达 M61=1；失败时返回架构评审，禁止用重复写入补偿。
...
M106、M107、M108 是按住写 1、松开写 0 的持续命令。M109 是保持型挡停命令。全部只允许管理员，并共同要求在线、手动模式 M1=1、M3=0、无急停 M0=0、无锁存故障 M14=0。M106/M107 还要求 M61=1 且 M50=0；M108/M109 不要求 M61，不得仅因尚未回原点而禁用。
...
通讯突然中断时 HMI 清零可能无法送达；HMI 不离线重放清零或重复写 1，重连后以真实 PLC 状态为准。

### 10.8 直通、皮带常转和安全屏蔽

M105 直通模式、M42 皮带常转、M110 光栅屏蔽、M111 门磁屏蔽只允许管理员修改并写入审计。

安全屏蔽必须二次确认，生效期间顶部持续显示琥珀色横幅。用户注销或会话超时时，HMI 在线时尝试清除 M50、M42、M106-M111；通讯中断时不离线重放。M105 模式选择保持不变。
...
命令不得乐观更新状态。启动等待 M3；复位是明确例外，只等待 M103/M50 的关联传输成功（M50 含写后回读）和 200 ms 最小延时，不用 M61 判定回原点完成；调宽等待 M44/M45，模式切换等待 M1/M2，保持写等待位或映射状态回读。
...
- 在线注销触发 M50、M42、M106-M111 清零流程；离线不重放。
...
应用正常退出时在在线情况下尝试清除 M50、M42、M106-M111，然后断开串口。通讯中断或异常退出时不离线重放清零写入。M100 在任何退出路径都不得自动清除。
